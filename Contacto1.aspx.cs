...
{
    public partial class Contacto1 : FormaPadre
    {
        ...
        protected void btnEnviar_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                ClientScript.RegisterStartupScript(this.GetType(), "PopupScript", "CloseFormOK();", true);
            }
            catch (Exception ex)
            {
                MessageBox(sender, e, ((Idioma)Session["oIdioma"]).Texto(ex.Message));
        ...
        private void AgregaScriptCliente()
        {
            try
            {
                ...
                codigo = "function CloseFormOK() { ";
                codigo += "  alert('" + ((Idioma)Session["oIdioma"]).Texto("MsgMsgRecibido") + "'); ";
                codigo += "  window.returnValue = true; ";
                codigo += "  self.close(); ";
                codigo += "} ";


                Page.ClientScript.RegisterClientScriptBlock(Page.GetType(), "CloseFormOK", codigo, true);

                codigo = "function CloseFormCancel() { ";
                codigo += "window.returnValue = false; ";
                codigo += "self.close(); ";
                codigo += "} ";

                Page.ClientScript.RegisterClientScriptBlock(Page.GetType(), "CloseFormCancel", codigo, true);
